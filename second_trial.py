...
from base_models.models import StudentModel, Req, StudentModelReply, UpdateStudentModel, responseWithKey, LoginModel, Encrypt, EncryptedText, DecryptedText, Decrypt, ImageFile, HideText
from typing import List, Union
import motor.motor_asyncio
from pymongo import MongoClient
import utilities.passwordUtil as passwordUtil
import jwt
from PIL import Image
import rsa
import base64
from utilities.sendEmail import sendMail, sendMailTwo, sendMailWithAttachment
...
async def create_student(student: StudentModel = Body(...)):
    ...
    if studentS is not None:
        ...
        student["password"] = passwordUtil.hash_password(student["password"])
...
async def create_student(student: LoginModel = Body(...)):
    ...
    if studentD is not None:
        passwordMatch =passwordUtil.verify_password(student['password'], studentD['password'])
...
async def list_students(token: Union[str, None] = Header(default=None), f5key: Union[str, None] = Header(default=None), textToHide: Union[str, None] = Header(default=None), email: Union[str, None] = Header(default=None), file: UploadFile = File(...)):
    ...
    if result is not None:
        ...
        try:
            ...
            message = "Hi {}, you have received an attachment. You f5 key is: ".format(studentD['username'])
            ...
            im1 = stepic.encode(image, textToHide.encode('utf-8'))
...
@app.post(
    "/seperate-image", response_description="List all students", response_model=ImageFile
)
